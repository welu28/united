interface GameResult {
  type: "notedown" | "trivia" | "flashcard"
  score: number
...
interface ActivityItem {
  timestamp: string
  description: string
  type: "notedown" | "trivia" | "flashcard" | "create" | "upload" | "profile"
  score?: number
  studySetId?: string
}
...
export function logGameResult(result: GameResult) {
  ...
  switch (result.type) {
    case "notedown":
      description = `Completed NoteDown session - ${accuracy}% accuracy (${result.correctAnswers}/${result.questionsAnswered})`
      break
    case "trivia":
