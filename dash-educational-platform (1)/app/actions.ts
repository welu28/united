...
export async function generateQuestionsAction(text: string): Promise<QuestionAnswer[]> {
  try {
    ...
    // --- NEW robust JSON extraction/parsing ---
    // The model sometimes adds a preamble like "Here is the JSON ..."
    // or wraps the payload in \`\`\`json fences.  We extract the array
    // between the first "[" and the last "]" and parse only that part.
    let raw = generatedText.trim()

    // strip triple-back-tick fences if present
    raw = raw
      .replace(/```(?:json)?/gi, "")
      .replace(/```/g, "")
      .trim()
    ...
    // --- END robust JSON extraction/parsing ---
  } catch (error) {
...
export async function checkSemanticSimilarity(userAnswer: string, correctAnswer: string): Promise<boolean> {
  try {
    ...
    const { text: result } = await generateText({
      model: groq("llama3-70b-8192"),
      prompt,
      temperature: 0.1, // Keep temperature low for factual judgment
    })
    ...
    // Fallback to a strict comparison or handle as incorrect if AI fails
